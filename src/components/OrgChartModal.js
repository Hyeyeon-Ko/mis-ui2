...
const OrgChartModal = ({ show, onClose, onConfirm, renderOrgTree, selectedUsers, setSelectedUsers, instCd, teamMembers = []}) => {
      ...
      axios.get('/api/info/confirm', { params: { instCd: auth.instCd } }) 
        .then(response => {
          if (response.data && response.data.data) {
            const { teamLeaderId, teamLeaderNm, teamLeaderRoleNm, teamLeaderPositionNm, teamLeaderDept, managerId, managerNm, managerRoleNm, managerPositionNm, managerDept } = response.data.data[0];
            ...
            setSelectedUsers([manager, teamLeader]); 
          }
        })
        .catch(error => {
  ...
  }, [show, auth.instCd, setSelectedUsers]);
  ...
  const handleReset = () => {
    setSelectedUsers((prevSelectedUsers) =>
      prevSelectedUsers.filter((user) => user.seq === 2 || user.seq === 3)
    );
    setSelectedRows([]);
  };
        ...
        <div className="orgChart-modal-body">
          ...
          <div className="orgChart-selection">
            ...
            <div className="selection-guidance-container">
              ...
              <p className="selection-guidance-body">명함 대상자(본인) 부서의 팀장님을 선택하세요.</p>
              <p className="selection-guidance-body">2, 3번은 경영지원팀/총무팀 담당자로 자동 설정되어 있습니다.</p>
            </div>
          </div>
        </div>
